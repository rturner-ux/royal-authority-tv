...
export async function generateMetadata({
  ...
  const { slug, citySlug: targetCitySlug } = await params;
  const collection = getCollection(slug);
  if (!collection) return { title: "Royal Authority TV" };

  const cases = await getCasesByCollection(slug);
  const match = cases.find((c) => c.city && c.state && citySlug(c.city, c.state) === targetCitySlug);
  const cityName = match ? `${match.city}, ${match.state}` : "";

  return {
    title: `${collection.name}${cityName ? ` in ${cityName}` : ""} | Royal Authority TV`,
    description: collection.description,
  };
}

export default async function CollectionCityPage({
  params,
}: {
  params: Promise<{ slug: string; citySlug: string }>;
}) {
  ...
  const cases = allCases.filter(
    (c) => c.city && c.state && citySlug(c.city, c.state) === targetCitySlug
  );
  if (cases.length === 0) notFound();

  const cityName = `${cases[0].city}, ${cases[0].state}`;
    ...
    <main className="relative min-h-screen bg-[#05070b] text-white overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6 py-6">
        ...
        <div className="mb-12">
          ...
          <h1 className="mt-3 text-4xl font-bold md:text-5xl">{cityName}</h1>
          <p className="mt-3 max-w-2xl text-gray-400">
            {cases.length} {cases.length === 1 ? "case" : "cases"} in this collection from {cityName}.
          </p>
        </div>
