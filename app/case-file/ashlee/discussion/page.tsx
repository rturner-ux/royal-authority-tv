...
const starterThreads: Thread[] = [
  {
    ...
    tag: "Moderator Note",
    title: "Discussion Guidelines for the Ashlee Case",
    body:
      "Keep discussion factual, respectful, and tied to the available record. Do not post private information, unsupported accusations as fact, or harassment toward individuals connected to the case.",
    time: "Pinned",
    pinned: true,
    replies: [
      {
        id: 101,
        author: "Subscriber 03",
        body:
          "Understood. Keeping discussion tied to the transcript and timeline is the right move.",
        time: "1h ago",
      },
    ],
  },
  {
    id: 2,
    author: "Subscriber 14",
    tag: "Timeline",
    title: "The 20-minute window feels more important after seeing the resort layout",
    body:
      "The villa spacing changes everything for me. On a property this spread out, 20 minutes is not the same as 20 minutes inside a normal hotel. It makes me want to see a more detailed movement reconstruction.",
    time: "2h ago",
    replies: [],
  },
  {
    id: 3,
    author: "Subscriber 27",
    tag: "Transcript",
    title: "The charger detail stands out",
    body:
      "The staff interaction around the charger feels like one of the most important details in the statement. It may help narrow the timeline more than people realize.",
    time: "4h ago",
    replies: [],
  },
  {
    id: 4,
    author: "Subscriber 08",
    tag: "Scene Analysis",
    title: "Villa 25 and 65 being separated matters",
    body:
      "If Villa 65 was really that far from Villa 25, that changes how people should think about proximity and access after the reported relocation.",
    time: "6h ago",
    replies: [],
  },
];
...
export default function AshleeDiscussionPage() {
  ...
  function handleReplyChange(threadId: number, value: string) {
  ...
  function handleUpvote(threadId: number) {
  ...
  function handleSave(threadId: number) {
  ...
  const filteredThreads = useMemo(() => {
    const filtered = threads.filter((thread) => {
      const replyText = thread.replies.map((r) => r.body).join(" ");
      const haystack =
        `${thread.author} ${thread.tag} ${thread.title} ${thread.body} ${replyText}`.toLowerCase();
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 py-6 lg:px-10">
        ...
        <div className="mb-10">
          ...
          <h1 className="mt-3 font-serif text-4xl text-white md:text-5xl">
            Ashlee Case Discussion Room
          </h1>
          <p className="mt-4 max-w-4xl text-sm leading-8 text-slate-300 md:text-base">
            Open discussion space for case observations, timeline questions,
            transcript analysis, and scene-layout debate. Keep discussion factual,
            respectful, and tied to available information.
          </p>
        </div>

        <section className="mb-8 grid gap-4 md:grid-cols-3">
          <div className="rounded-[24px] border border-white/10 bg-black/30 p-5 backdrop-blur-sm">
            <div className="text-xs uppercase tracking-[0.22em] text-[#E8D19A]">
              Active Threads
            </div>
          ...
          <div className="rounded-[24px] border border-white/10 bg-black/30 p-5 backdrop-blur-sm">
            ...
            <div className="mt-3 text-lg font-semibold text-red-300">
              Open and Active
            </div>
          </div>
        </section>
          ...
          <div className="rounded-[30px] border border-white/10 bg-black/30 p-6 backdrop-blur-sm">
            ...
            <div className="mt-5 grid gap-3 text-sm text-slate-300">
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4">
                Discuss evidence, timeline, transcript details, and scene layout.
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4">
                Focus on verified information and clearly label speculation.
              </div>
            </div>
          ...
          <div className="rounded-[30px] border border-white/10 bg-black/30 p-6 backdrop-blur-sm">
            ...
            <form onSubmit={handlePost} className="space-y-4">
              ...
              <textarea
                ...
                rows={6}
                placeholder="Share your question, timeline observation, transcript point, or case theory..."
          ...
          {filteredThreads.map((thread) => (
            ...
            >
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <button
                  onClick={() =>
                    setOpenReplyId(openReplyId === thread.id ? null : thread.id)
                  }
                  className="rounded-2xl border border-white/15 bg-white/5 px-4 py-2 text-sm font-semibold text-white transition hover:bg-white/10"
                >
              ...
              {openReplyId === thread.id && (
                <div className="mt-5 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                  <div className="mb-3 text-xs uppercase tracking-[0.2em] text-[#E8D19A]">
                    Add Reply
                  </div>
