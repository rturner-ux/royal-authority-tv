...
export default async function ModeratorsPage() {
  ...
  if (!user) redirect("/login?next=/account/admin/moderators");
    ...
    <main className="relative min-h-screen bg-[#05070b] text-white">
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-6 pb-20 pt-6">
        <div className="lg:hidden">
          <Navbar breadcrumbs={[{ label: "Home", href: "/" }, { label: "Profile", href: "/account" }, { label: "Moderators" }]} />
        </div>

        <div className="text-xs uppercase tracking-[0.34em] text-[#E8D19A]">Admin</div>
        <h1 className="mt-2 font-serif text-3xl font-bold text-white">Manage Moderators</h1>
        <p className="mt-2 text-sm leading-6 text-slate-400">
          Grant or remove moderator status for any member with a callsign.
        </p>
