...
export default function AlprRoutePlanner() {
  const map = useMap();
  ...
  useMapEvents({
    click(e) {
      if (!picking) return;
      const p = { lat: e.latlng.lat, lng: e.latlng.lng, label: `${e.latlng.lat.toFixed(4)}, ${e.latlng.lng.toFixed(4)}` };
  ...
  async function getRoute() {
    ...
    try {
      ...
      if (d.success) {
        ...
        const bounds: [number, number][] = d.direct.geometry.map((c: [number, number]) => [c[1], c[0]]);
  ...
  return (
    <>
      ...
      {result && (
        <>
          <Polyline
            positions={result.direct.geometry.map((c) => [c[1], c[0]])}
            pathOptions={{ color: "#f97316", weight: 4, opacity: 0.85 }}
          />
          {result.privacy && (
            <Polyline
              positions={result.privacy.geometry.map((c) => [c[1], c[0]])}
